...
Jobs are executed synchronously by default and the same [runner](https://openagentsinc.github.io/openagents-node-python-sdk/JobRunner.html#openagents.JobRunner.JobRunner.setRunInParallel) instance processes every job.

- `setRunInParallel(False)` it will do it synchronously.

- `setRunInParallel(True)` will do it in parallel.
