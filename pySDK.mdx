...
Initialization of everything is done on `init`.
...
Jobs are executed synchronously by default and the same  Runner instance processes every job.
...
- if `setRunInParallel(False)` it will do it synchronously.

- if `setRunInParallel(True)` will do it in parallel.
